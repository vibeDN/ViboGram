...
- [x] Customizable "edited" / "deleted" message marks. Edited label is user-customizable text; anti-deleted messages (Tier 3) get a 🗑 glyph in the same label slot instead, plus the whole bubble rendered semi-transparent — for text messages only so far (other content types like media/polls/stickers don't show the visual treatment yet, still needs porting per-content-type).
...
- [x] Local anti-delete (keep local history of deleted/edited messages), behind a settings toggle; secret chats are excluded. A message another side deletes in a cloud chat/channel is kept (tagged, not removed) instead of vanishing, is permanently protected from ever actually being purged (even a repeated delete request for the same message no longer undoes the "kept" state — an earlier version of this had that bug), gets a 🗑 + semi-transparent visual treatment in the chat bubble (text messages only so far), and is also permanently backed up to a local JSON store (`Swiftgram/SGMessageArchive`, in the shared App Group container) independent of Postbox's own database/cache lifecycle. Edited messages' prior text versions are similarly archived (separate file, same folder) — a "История" (history) view for browsing versions via long-press is not built yet. The originally-discussed hybrid design (also mirror deleted cloud-chat messages into Saved Messages, for cross-device sync) was dropped in favor of the local-file approach above, per explicit decision — cross-device sync is not a goal here.
...
- [x] Keep chat history after being banned/kicked from a chat. `shouldExcludePeerFromChatList` (`UpdatePeers.swift`) is the single shared predicate everywhere chat-list visibility gets decided; being `.kicked`/`.Removed` from a channel/group now keeps it visible behind a settings toggle. Voluntarily leaving still hides it as before — only involuntary removal is affected. Only applies going forward (doesn't retroactively un-hide chats hidden before the toggle was turned on).
- [x] Restore deleted gifts. Telegram's own servers still hold limited-edition gifts that have aged out of the live catalog (e.g. a past holiday gift) — the stock client just redirects a tap on one to a read-only info screen instead of the send flow. Behind a settings toggle, this lets you reach the normal send flow for those instead. Only bypasses the "no longer in the catalog" gate — not per-person purchase limits, and not unique/collectible gifts whose supply is genuinely exhausted (those stay blocked, correctly). The server independently re-validates on send either way, so this can only let you *attempt* it, not force through a sale Telegram has actually closed out.
- [x] Local Telegram Premium UI unlock, ported from AyuGram4A's `AyuConfig.localPremium` design: a settings toggle OR's `true` into the single chokepoint (`Peer.isPremium` in TelegramCore) every Premium-gated UI surface already reads, scoped to only your own logged-in account(s) — other people's Premium badges/status stay accurate, this never fakes anyone else. Purely cosmetic client-side gates (extra reactions, animated emoji status, expanded folder limits, etc.) — anything the server independently validates (real upload limits, sending Premium-only content) is still rejected server-side regardless.
- [x] Music file tag editing (title/artist/cover) for tracks already set on your own profile (Saved Music). A new edit button next to the existing add button in the overlay music player prompts for title, artist and (optionally) a new cover, then re-uploads the already-cached audio as a fresh standalone document with the new tags — `account.saveMusic` itself has no tag-editing parameters, so this is the only way to change them. The old track is removed and the new one added in its place; exact list position isn't preserved (by design, kept simple).
- [ ] User badges next to name/in profile (owner-curated list + optional custom image, tap-to-view with a 3D spin), sourced from a repo secret. Also fetch and display badges from Margelet's own public `badges.json` for interop. Partial: data layer is done and live (`Swiftgram/SGBadges` — own repo-secret JSON + Margelet's `badges.json`, cached), and a first-cut UI landed — a small colored marker next to the name in the profile header (own-profile and others'), tap opens a plain title/about popup. Deliberately deferred without a device to visually verify against: the badge *image* (`img_url`) isn't rendered yet (marker is just a solid color swatch), and there's no 3D spin/tap interaction — no ready-made component for that exists in this codebase (closest precedent, `CubeAnimationView`, is a full 6-face gift-crafting cube, overbuilt for this), so it needs a small purpose-built gesture-driven `CATransform3D` view, built and tuned live once there's on-device access.
