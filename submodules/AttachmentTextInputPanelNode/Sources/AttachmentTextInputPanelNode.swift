...
public class AttachmentTextInputPanelNode: ASDisplayNode, TGCaptionPanelView, ASEditableTextNodeDelegate, ChatInputTextNodeDelegate {
    ...
    public func editableTextNodeTarget(forAction action: Selector) -> ASEditableTextNodeTargetForAction? {
        if action == makeSelectorFromString("_accessibilitySpeak:") {
            ...
            // MARK: ViboGram - see the same fix in ChatTextInputPanelNode.swift
            // -- Dim/Rainbow/SizeBig existed as working @objc handlers below
            // but were never in this list, so UIKit would never treat a tap
            // on them (once TextInputMenu.swift's menu items are fixed to
            // even show them) as having a valid target.
            if case .format = self.inputMenu.state {
                return ASEditableTextNodeTargetForAction(target: self)
            } else {
                return ASEditableTextNodeTargetForAction(target: nil)
    ...
    @objc func formatAttributesDim(_ sender: Any) {
        self.inputMenu.back()
        self.interfaceInteraction?.updateTextInputStateAndMode { current, inputMode in
    ...
    @objc func formatAttributesRainbow(_ sender: Any) {
        self.inputMenu.back()
        self.interfaceInteraction?.updateTextInputStateAndMode { current, inputMode in
    ...
    // instead of one fixed "Big" toggle. Same preset-picker approach as
    // ChatTextInputPanelNode.swift's counterpart -- see its comment for why
    // a plain vertical textAlertController was chosen over a custom slider.
    @objc func formatAttributesSizeBig(_ sender: Any) {
        self.inputMenu.back()
